...
/// Human-readable runtime status used by existing routine list surfaces.
pub fn routine_status_label(routine: &Routine) -> &'static str {
    if !routine.enabled {
        "disabled"
    } else if routine.consecutive_failures > 0 {
        "failing"
    } else {
        "active"
    }
...
/// Health-oriented status for operator status surfaces.
pub fn routine_health_label(routine: &Routine) -> &'static str {
    if !routine.enabled {
        "disabled"
    } else if routine.consecutive_failures == 0 {
        "healthy"
...
/// Resolve the routine trigger channel for visibility surfaces.
pub fn routine_trigger_channel(trigger: &Trigger) -> Option<String> {
    match trigger {
        Trigger::Event {
            channel: Some(channel),
            ..
        } => Some(channel.clone()),
        Trigger::Event { channel: None, .. } => Some("any".to_string()),
        Trigger::Webhook { .. } => Some("webhook".to_string()),
        _ => None,
    }
}

/// Resolve the effective webhook path (defaulting to `/hooks/routine/{id}`).
pub fn routine_webhook_path(routine_id: Uuid, trigger: &Trigger) -> Option<String> {
    match trigger {
        Trigger::Webhook { path, .. } => Some(
            path.clone()
                .unwrap_or_else(|| format!("/hooks/routine/{routine_id}")),
        ),
        _ => None,
    }
}
...
mod tests {
    ...
    use super::{
        routine_health_label, routine_status_label, routine_trigger_channel, routine_webhook_path,
    };
    ...
    #[test]
    fn test_routine_status_and_health_labels() {
        let now = Utc::now();
        let mut routine = Routine {
            id: Uuid::new_v4(),
            name: "nightly".to_string(),
            description: "nightly checks".to_string(),
        ...
        assert_eq!(routine_status_label(&routine), "active");
        assert_eq!(routine_health_label(&routine), "healthy");

        routine.consecutive_failures = 1;
        assert_eq!(routine_status_label(&routine), "failing");
        assert_eq!(routine_health_label(&routine), "degraded");

        routine.consecutive_failures = 4;
        assert_eq!(routine_health_label(&routine), "failing");

        routine.enabled = false;
        assert_eq!(routine_status_label(&routine), "disabled");
        assert_eq!(routine_health_label(&routine), "disabled");
    }

    #[test]
    fn test_routine_trigger_channel_and_webhook_path() {
        let event = Trigger::Event {
            channel: Some("telegram".to_string()),
            pattern: "deploy".to_string(),
        };
        assert_eq!(routine_trigger_channel(&event).as_deref(), Some("telegram"));

        let any_event = Trigger::Event {
            channel: None,
            pattern: "deploy".to_string(),
        };
        assert_eq!(routine_trigger_channel(&any_event).as_deref(), Some("any"));
        ...
        assert_eq!(
            routine_trigger_channel(&webhook).as_deref(),
            Some("webhook")
        );
        assert_eq!(
            routine_webhook_path(Uuid::nil(), &webhook).as_deref(),
            Some("/hooks/routine/00000000-0000-0000-0000-000000000000")
        );
    }
}
