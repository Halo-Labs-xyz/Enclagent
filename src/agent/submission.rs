...
fn canonical_system_command(token_lower: &str) -> Option<&'static str> {
    match token_lower {
        "/help" | "/?" => Some("help"),
        ...
        "/debug" => Some("debug"),
        "/model" => Some("model"),
        "/positions" | "/position" | "/pos" => Some("positions"),
        "/exposure" | "/expo" => Some("exposure"),
        "/funding" | "/fund" | "/funds" => Some("funding"),
        "/vault" | "/wallet-vault" => Some("vault"),
        "/risk" | "/riskbook" => Some("risk"),
        "/pause-agent" | "/pause_agent" | "/pauseagent" | "/agent-pause" => Some("pause-agent"),
        "/resume-agent" | "/resume_agent" | "/resumeagent" | "/agent-resume" => {
            Some("resume-agent")
        }
        "/verify" | "/verify-receipt" => Some("verify"),
        "/receipts" | "/receipt-log" => Some("receipts"),
        _ => None,
    }
}
...
impl SubmissionParser {
    ...
    pub fn parse(content: &str) -> Submission {
        ...
        let first_lower = parts.first().map(|p| p.to_ascii_lowercase());

        if let Some(first) = first_lower.as_deref() {
            // Control commands
            if first == "/undo" {
            ...
            if first == "/summarize" || first == "/summary" {
            ...
            if first == "/new"
                || (first == "/thread"
                    && parts
                        .get(1)
                        .is_some_and(|arg| arg.eq_ignore_ascii_case("new")))
            {
                return Submission::NewThread;
            }

            // System commands (bypass thread-state checks)
            if let Some(command) = canonical_system_command(first) {
            ...
            if first == "/quit" || first == "/exit" || first == "/shutdown" {
                return Submission::Quit;
            }

            // /thread <uuid> - switch thread
            if first == "/thread"
                && let Some(thread_id) = parts.get(1)
                && !thread_id.eq_ignore_ascii_case("new")
                && let Ok(id) = Uuid::parse_str(thread_id)
            {
            ...
            if first == "/resume"
                && let Some(checkpoint_id) = parts.get(1)
                && let Ok(id) = Uuid::parse_str(checkpoint_id)
            {
...
mod tests {
    ...
    fn test_parser_ws2_commands() {
        for (raw, expected) in [
            ...
            let submission = SubmissionParser::parse(raw);
            assert!(
                matches!(&submission, Submission::SystemCommand { command, args } if command == expected && args.is_empty()),
    ...
    #[test]
    fn test_parser_ws2_command_aliases() {
        for (raw, expected) in [
            ("/pos", "positions"),
            ("/position", "positions"),
            ("/expo", "exposure"),
            ...
            let submission = SubmissionParser::parse(raw);
            assert!(
                matches!(&submission, Submission::SystemCommand { command, .. } if command == expected),
                "expected {:?} for {} but got {:?}",
                expected,
                raw,
                submission
    ...
    #[test]
    fn test_parser_ws2_commands_with_args() {
        let verify = SubmissionParser::parse("   /VERIFY   receipt-123   ");
        assert!(
            matches!(verify, Submission::SystemCommand { command, args } if command == "verify" && args == vec!["receipt-123"])
        );

        let receipts = SubmissionParser::parse("/receipts Agent-XYZ");
        assert!(
            matches!(receipts, Submission::SystemCommand { command, args } if command == "receipts" && args == vec!["Agent-XYZ"])
        );
    }

    #[test]
    fn test_parser_resume_checkpoint_does_not_conflict_with_resume_agent() {
        let checkpoint = Uuid::new_v4();
        let submission = SubmissionParser::parse(&format!("/resume {}", checkpoint));
        assert!(
            matches!(submission, Submission::Resume { checkpoint_id } if checkpoint_id == checkpoint)
        );

        let resume_agent = SubmissionParser::parse("/resume-agent");
        assert!(
            matches!(resume_agent, Submission::SystemCommand { command, args } if command == "resume-agent" && args.is_empty())
        );
    }
