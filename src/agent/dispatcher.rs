...
const MAX_TOOL_ITERATIONS_CAP: usize = 200;

fn resolve_max_tool_iterations() -> usize {
    let Ok(raw) = std::env::var("AGENT_MAX_TOOL_ITERATIONS") else {
    ...
    let parsed = match trimmed.parse::<usize>() {
        ...
        Err(_) => {
            tracing::warn!(
                ...
                "Invalid AGENT_MAX_TOOL_ITERATIONS; using default"
            );
            return DEFAULT_MAX_TOOL_ITERATIONS;
        }
    };
    ...
    if parsed == 0 {
        tracing::warn!(
            default = DEFAULT_MAX_TOOL_ITERATIONS,
            "AGENT_MAX_TOOL_ITERATIONS must be > 0; using default"
        );
        return DEFAULT_MAX_TOOL_ITERATIONS;
    }

    if parsed > MAX_TOOL_ITERATIONS_CAP {
        tracing::warn!(
            requested = parsed,
            capped = MAX_TOOL_ITERATIONS_CAP,
            "AGENT_MAX_TOOL_ITERATIONS exceeds cap; applying cap"
        );
        return MAX_TOOL_ITERATIONS_CAP;
    }
...
impl Agent {
    ...
    pub(super) async fn run_agentic_loop(
        ...
        let max_tool_iterations = resolve_max_tool_iterations();
        let mut iteration = 0;
...
mod max_iteration_tests {
    ...
    fn clear_env() {
        // SAFETY: Env mutation is protected by ENV_MUTEX in tests.
        unsafe {
    ...
    #[test]
    fn max_tool_iterations_defaults_when_unset() {
        let _guard = ENV_MUTEX.lock().expect("env mutex poisoned");
        clear_env();
        assert_eq!(resolve_max_tool_iterations(), DEFAULT_MAX_TOOL_ITERATIONS);
    }

    #[test]
    fn max_tool_iterations_uses_valid_env_value() {
        let _guard = ENV_MUTEX.lock().expect("env mutex poisoned");
        clear_env();
        // SAFETY: Env mutation is protected by ENV_MUTEX in tests.
        unsafe {
            std::env::set_var("AGENT_MAX_TOOL_ITERATIONS", "64");
        }
        assert_eq!(resolve_max_tool_iterations(), 64);
        clear_env();
    }

    #[test]
    fn max_tool_iterations_defaults_on_invalid_env_value() {
        let _guard = ENV_MUTEX.lock().expect("env mutex poisoned");
        clear_env();
        // SAFETY: Env mutation is protected by ENV_MUTEX in tests.
        unsafe {
            std::env::set_var("AGENT_MAX_TOOL_ITERATIONS", "nope");
        }
        assert_eq!(resolve_max_tool_iterations(), DEFAULT_MAX_TOOL_ITERATIONS);
        clear_env();
    }
    ...
    #[test]
    fn max_tool_iterations_caps_large_env_value() {
        let _guard = ENV_MUTEX.lock().expect("env mutex poisoned");
        clear_env();
        // SAFETY: Env mutation is protected by ENV_MUTEX in tests.
        unsafe {
            std::env::set_var("AGENT_MAX_TOOL_ITERATIONS", "999");
        }
        assert_eq!(resolve_max_tool_iterations(), MAX_TOOL_ITERATIONS_CAP);
        clear_env();
    }
}
