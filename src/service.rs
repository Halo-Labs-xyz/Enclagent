//! OS service management for running Enclagent as a daemon.
//!
//! Generates and manages platform-native service definitions:
//! - **macOS**: launchd plist at `~/Library/LaunchAgents/com.enclagent.daemon.plist`
//! - **Linux**: systemd user unit at `~/.config/systemd/user/enclagent.service`
//!
//! The installed service runs `enclagent run` (the default agent mode) and is
...
const SERVICE_LABEL: &str = "com.enclagent.daemon";
const SYSTEMD_UNIT: &str = "enclagent.service";
...
fn install_macos() -> Result<()> {
    ...
    let logs_dir = enclagent_logs_dir()?;
...
"#,
    ...
    println!("  Start with: enclagent service start");
    Ok(())
}
...
fn start() -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        if !plist.exists() {
            bail!("Service not installed. Run `enclagent service install` first.");
        }
...
fn enclagent_logs_dir() -> Result<PathBuf> {
    let home = dirs::home_dir().context("could not find home directory")?;
    Ok(home.join(".enclagent").join("logs"))
}
...
mod tests {
    ...
    fn macos_plist_path_has_expected_suffix() {
        ...
        assert!(
            s.ends_with("Library/LaunchAgents/com.enclagent.daemon.plist"),
            "unexpected path: {s}"
        );
    }
    ...
    fn linux_unit_path_has_expected_suffix() {
        ...
        let s = path.to_string_lossy();
        assert!(
            s.ends_with(".config/systemd/user/enclagent.service"),
            "unexpected path: {s}"
        );
    }

    #[test]
    fn logs_dir_under_enclagent() {
        let path = enclagent_logs_dir().unwrap();
        let s = path.to_string_lossy();
        assert!(s.ends_with(".enclagent/logs"), "unexpected path: {s}");
    }
}
