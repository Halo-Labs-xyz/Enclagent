...
pub trait SecretsStore: Send + Sync {
    ...
    /// Resolve and decrypt verification credentials for Eigen jobs.
    async fn get_verification_job_secret(
        &self,
        user_id: &str,
        credential_ref: &VerificationJobCredentialRef,
    ) -> Result<DecryptedSecret, SecretError> {
        credential_ref.validate()?;
        self.get_decrypted(user_id, &credential_ref.secret_name)
            .await
    }
}
