        ...
        for (secret_name, env_var_value) in creds_obj {
            ...
            if !exists {
                return Err(ToolError::ExecutionFailed(format!(
                    "secret '{}' not found. Store it first via 'enclagent tool auth' or the web UI.",
                    secret_name
                )));
            }
...
fn resolve_project_dir_with_base(
...
///
/// Auto-creates `~/.enclagent/projects/{project_id}/` so every sandbox job has a
...
/// it is validated to fall within `~/.enclagent/projects/` after canonicalization.
fn resolve_project_dir(
    explicit: Option<PathBuf>,
    project_id: Uuid,
) -> Result<(PathBuf, String), ToolError> {
    let base = projects_base();
    resolve_project_dir_with_base(explicit, project_id, &base)
}
            ...
            serde_json::json!({
                ...
                "properties": {
                    ...
                    "mode": {
                        ...
                        "description": "Execution mode. 'worker' (default) uses the Enclagent sub-agent. \
                    ...
                    "project_dir": {
                        ...
                        "description": "Path to an existing project directory to mount into the container. \
                                        Must be under ~/.enclagent/projects/. If omitted, a fresh directory is created."
                    },
                    "credentials": {
                        "type": "object",
                        "description": "Map of secret names to env var names. Each secret must exist in the \
                                        secrets store (via 'enclagent tool auth' or web UI). Example: \
...
mod tests {
    ...
    fn test_resolve_project_dir_auto() {
        ...
        let base = base_tmp.path().join("projects");
        let (dir, browse_id) = resolve_project_dir_with_base(None, project_id, &base).unwrap();
        assert!(dir.exists());
    ...
    fn test_resolve_project_dir_explicit_under_base() {
        ...
        let (dir, browse_id) =
            resolve_project_dir_with_base(Some(explicit.clone()), project_id, &base).unwrap();
        assert!(dir.exists());
    ...
    fn test_resolve_project_dir_rejects_outside_base() {
        ...
        let result = resolve_project_dir_with_base(Some(escape_attempt), Uuid::new_v4(), &base);
    ...
    fn test_resolve_project_dir_rejects_outside_base_existing() {
        ...
        let result = resolve_project_dir_with_base(Some(outside), Uuid::new_v4(), &base);
        assert!(result.is_err());
        let err = result.unwrap_err().to_string();
        assert!(
    ...
    fn test_resolve_project_dir_rejects_traversal() {
        ...
        let result = resolve_project_dir_with_base(Some(traversal), Uuid::new_v4(), &base);
        ...
        let base_parent = base.join("..").join("definitely_not_projects");
        std::fs::create_dir_all(&base_parent).ok();
        if base_parent.exists() {
            let result =
                resolve_project_dir_with_base(Some(base_parent.clone()), Uuid::new_v4(), &base);
