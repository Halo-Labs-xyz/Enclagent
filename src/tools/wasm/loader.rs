...
//! loader.load_from_dir(Path::new("~/.enclagent/tools/")).await?;
//! ```
//!
//! # Dev Mode
//!
...
///
/// Checks (in order):
/// 1. `ENCLAGENT_TOOLS_SRC` env var
/// 2. `<CARGO_MANIFEST_DIR>/tools-src/` (dev builds)
fn tools_src_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("ENCLAGENT_TOOLS_SRC") {
...
/// needing to install them to `~/.enclagent/tools/` first. Build artifacts
/// that are newer than installed copies take priority.
///
/// Set `ENCLAGENT_TOOLS_SRC` env var to override the source directory.
