...
use enclagent::{
    agent::{Agent, AgentDeps, SessionManager as AgentSessionManager},
    channels::{
    ...
    llm::{
        ...
        SessionManager as LlmSessionManager, create_cheap_llm_provider, create_llm_provider,
...
#[cfg(feature = "libsql")]
use enclagent::secrets::LibSqlSecretsStore;
#[cfg(feature = "postgres")]
use enclagent::secrets::PostgresSecretsStore;
use enclagent::secrets::SecretsCrypto;
#[cfg(any(feature = "postgres", feature = "libsql"))]
use enclagent::setup::{SetupConfig, SetupWizard};

fn embeddings_dimension_for_model(model: &str) -> usize {
    match model {
...
fn normalize_embeddings_provider(provider: &str) -> &'static str {
    if provider.eq_ignore_ascii_case("nearai") {
        "nearai"
    } else {
        "openai"
    }
...
fn build_embeddings_provider(
    ...
    match provider {
        "nearai" => Some(Arc::new(
            NearAiEmbeddings::new(&config.llm.nearai.base_url, session)
                .with_model(model, embeddings_dimension_for_model(model)),
        )),
        "openai" => config.embeddings.openai_api_key().map(|api_key| {
            Arc::new(OpenAiEmbeddings::with_model(
                api_key,
                model,
                embeddings_dimension_for_model(model),
...
async fn init_embeddings_with_health_fallback(
    config: &Config,
    session: Arc<LlmSessionManager>,
    startup_scope: &str,
) -> Option<Arc<dyn EmbeddingProvider>> {
    if !config.embeddings.enabled {
        tracing::info!(
            scope = startup_scope,
            "Embeddings disabled (set OPENAI_API_KEY or EMBEDDING_ENABLED=true)"
        );
        return None;
    }

    let selected_provider = normalize_embeddings_provider(&config.embeddings.provider);
    if !config
        .embeddings
        .provider
        .eq_ignore_ascii_case(selected_provider)
    {
        tracing::warn!(
            scope = startup_scope,
            configured_provider = %config.embeddings.provider,
            resolved_provider = %selected_provider,
            "Unknown embeddings provider; defaulting to OpenAI"
        );
    }

    let selected_model = config.embeddings.model.as_str();
    let Some(provider) =
        build_embeddings_provider(config, session.clone(), selected_provider, selected_model)
    else {
        if selected_provider == "openai" {
            tracing::warn!(
                scope = startup_scope,
                "Embeddings configured for OpenAI but OPENAI_API_KEY is not set; disabling embeddings"
            );
        } else {
            tracing::warn!(
                scope = startup_scope,
                provider = %selected_provider,
                "Failed to construct embeddings provider; disabling embeddings"
            );
        }
        return None;
    };
    ...
    match provider.health_probe().await {
        Ok(()) => {
            tracing::info!(
                ...
                provider = %selected_provider,
                model = %selected_model,
                "Embeddings health probe succeeded"
            );
            Some(provider)
        }
        Err(error) => {
            tracing::warn!(
                ...
                model = %selected_model,
                error = %error,
                "Embeddings health probe failed"
            );

            let fallback_model = "text-embedding-3-small";
            if selected_model != fallback_model {
                tracing::warn!(
                    ...
                    fallback_model,
                    "Trying provider fallback model after failed embeddings health probe"
                );

                if let Some(fallback_provider) =
                    build_embeddings_provider(config, session, selected_provider, fallback_model)
                {
                    match fallback_provider.health_probe().await {
                        Ok(()) => {
                            tracing::warn!(
                                scope = startup_scope,
                                provider = %selected_provider,
                                model = fallback_model,
                                "Embeddings fallback model activated"
                            );
                            return Some(fallback_provider);
                        }
                        Err(fallback_error) => {
                            tracing::warn!(
                                scope = startup_scope,
                                ...
                                "Embeddings fallback model health probe failed"
                            );
                        }
                    }
                }
            ...
            tracing::warn!(
                scope = startup_scope,
                provider = %selected_provider,
                "Disabling embeddings after provider health probe failure"
            );
            None
        }
    }
...
fn enforce_workspace_startup_guardrails(command: &Option<Command>) -> anyhow::Result<()> {
    let applies = matches!(
        command,
        None | Some(Command::Run) | Some(Command::Doctor { .. })
    );
    if !applies {
        return Ok(());
    }
    ...
    let probable_workspace_root =
        cwd.join("enclagent").join("Cargo.toml").exists() && !cwd.join("Cargo.toml").exists();
    ...
    anyhow::bail!(
        "Detected workspace root ({}).\nRun Enclagent from its repository:\n  cd enclagent && ~/.cargo/bin/cargo +1.92.0 run -- run\nStartup preflight:\n  cd enclagent && ~/.cargo/bin/cargo +1.92.0 run -- doctor startup",
        cwd.display()
    );
}
...
async fn main() -> anyhow::Result<()> {
    let cli = Cli::parse();
    enforce_workspace_startup_guardrails(&cli.command)?;
    ...
    match &cli.command {
        ...
        Some(Command::Config(config_cmd)) => {
            ...
            return enclagent::cli::run_config_command(config_cmd.clone()).await;
        }
        ...
        Some(Command::Memory(mem_cmd)) => {
            ...
            let session = enclagent::llm::create_session_manager(enclagent::llm::SessionConfig {
                auth_base_url: config.llm.nearai.auth_base_url.clone(),
                session_path: config.llm.nearai.session_path.clone(),
            })
            .await;

            let embeddings = init_embeddings_with_health_fallback(&config, session, "memory").await;

            // Create a Database-trait-backed workspace for the memory command
            let db: Arc<dyn enclagent::db::Database> =
                enclagent::db::connect_from_config(&config.database)
                    .await
                    .map_err(|e| anyhow::anyhow!("{}", e))?;

            return enclagent::cli::run_memory_command_with_db(mem_cmd.clone(), db, embeddings)
                .await;
        }
        ...
        Some(Command::Doctor { command, strict }) => {
            tracing_subscriber::fmt()
                .with_env_filter(
                    EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("warn")),
                )
            ...
            return enclagent::cli::run_doctor_command(command.clone(), *strict).await;
        }
        Some(Command::Status) => {
            tracing_subscriber::fmt()
                .with_env_filter(
        ...
        Some(Command::Worker {
            ...
            tracing_subscriber::fmt()
                .with_env_filter(
                    EnvFilter::try_from_default_env()
                        .unwrap_or_else(|_| EnvFilter::new("enclagent=info")),
                )
                .init();
            ...
            let config = enclagent::worker::runtime::WorkerConfig {
                job_id: *job_id,
            ...
            let runtime = enclagent::worker::WorkerRuntime::new(config)
        ...
        Some(Command::ClaudeBridge {
            ...
            let config = enclagent::worker::claude_bridge::ClaudeBridgeConfig {
                job_id: *job_id,
                orchestrator_url: orchestrator_url.clone(),
                max_turns: *max_turns,
                model: model.clone(),
                timeout: std::time::Duration::from_secs(1800),
                allowed_tools: enclagent::config::ClaudeCodeConfig::from_env().allowed_tools,
            };

            let runtime = enclagent::worker::ClaudeBridgeRuntime::new(config)
        ...
        Some(Command::Onboard {
            ...
            // Standard ./.env first (higher priority), then ~/.enclagent/.env.
            let _ = dotenvy::dotenv();
            enclagent::bootstrap::load_enclagent_env();
    ...
    let mut config = match Config::from_env_with_toml(toml_path).await {
        Ok(c) => c,
        Err(enclagent::error::ConfigError::MissingRequired { key, hint }) => {
            eprintln!("Configuration error: Missing required setting '{}'", key);
            eprintln!("  {}", hint);
            eprintln!();
            eprintln!(
                "Run 'enclagent onboard' to configure, or set the required environment variables."
            );
            std::process::exit(1);
        }
    ...
    let env_filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("enclagent=info,tower_http=warn"));
    ...
    tracing_subscriber::registry()
        ...
        .with(
            tracing_subscriber::fmt::layer()
                .with_target(false)
                .with_writer(enclagent::tracing_fmt::TruncatingStderr::default()),
        )
    ...
    tracing::info!("Starting Enclagent...");
    ...
    // helper `enclagent::db::connect_from_config()`. This block is kept inline
    ...
    let db: Option<Arc<dyn enclagent::db::Database>> = if cli.no_db {
        tracing::warn!("Running without database connection");
        None
    } else {
        match config.database.backend {
            #[cfg(feature = "libsql")]
            enclagent::config::DatabaseBackend::LibSql => {
                use enclagent::db::Database as _;
                use enclagent::db::libsql::LibSqlBackend;
                use secrecy::ExposeSecret as _;

                let default_path = enclagent::config::default_libsql_path();
                let db_path = config
                    .database
                    .libsql_path
                    .as_deref()
                ...
                Some(Arc::new(backend) as Arc<dyn enclagent::db::Database>)
            }
            #[cfg(feature = "postgres")]
            _ => {
                use enclagent::db::Database as _;
                let pg = enclagent::db::postgres::PgBackend::new(&config.database)
                    .await
                    .map_err(|e| anyhow::anyhow!("{}", e))?;
                pg.run_migrations()
                    .await
                ...
                Some(Arc::new(pg) as Arc<dyn enclagent::db::Database>)
            }
    ...
    if let Some(ref db) = db {
        ...
        if let Err(e) = enclagent::bootstrap::migrate_disk_to_db(db.as_ref(), "default").await {
    ...
    if let Some(ref secrets) = secrets_store {
        enclagent::config::inject_llm_keys_from_secrets(secrets.as_ref(), "default").await;
    ...
    if config.llm.backend == enclagent::config::LlmBackend::NearAi
    ...
    let active_tunnel: Option<Box<dyn enclagent::tunnel::Tunnel>> =
        if config.tunnel.public_url.is_some() {
            ...
            match enclagent::tunnel::create_tunnel(provider_config) {
                Ok(Some(tunnel)) => {
                    tracing::info!(
                        "Starting {} tunnel on {}:{}...",
                        tunnel.name(),
    ...
    let embeddings: Option<Arc<dyn EmbeddingProvider>> =
        init_embeddings_with_health_fallback(&config, session.clone(), "runtime").await;
    ...
    let wasm_tools_future = async {
        if let Some(ref runtime) = wasm_tool_runtime {
            ...
            // Load installed tools from ~/.enclagent/tools/
    ...
    let mcp_servers_future = async {
        if let Some(ref secrets) = secrets_store {
            let servers_result = if let Some(ref d) = db {
                ...
                enclagent::tools::mcp::config::load_mcp_servers().await
            };
    ...
    let job_event_tx: Option<
        tokio::sync::broadcast::Sender<(uuid::Uuid, enclagent::channels::web::types::SseEvent)>,
    ...
    let prompt_queue = Arc::new(tokio::sync::Mutex::new(std::collections::HashMap::<
        uuid::Uuid,
        std::collections::VecDeque<enclagent::orchestrator::api::PendingPrompt>,
    >::new()));
    ...
    let container_job_manager: Option<Arc<ContainerJobManager>> = if config.sandbox.enabled {
        ...
        let job_config = ContainerJobConfig {
            ...
            claude_code_oauth_token: enclagent::config::ClaudeCodeConfig::extract_oauth_token(),
    ...
    let session_manager = Arc::new(AgentSessionManager::new().with_hooks(hooks.clone()));
    ...
    let (skill_registry, skill_catalog) = if config.skills.enabled {
        let mut registry = enclagent::skills::SkillRegistry::new(config.skills.local_dir.clone());
        ...
        let catalog = enclagent::skills::catalog::shared_catalog();
    ...
    let cost_guard = Arc::new(enclagent::agent::cost_guard::CostGuard::new(
        enclagent::agent::cost_guard::CostGuardConfig {
    ...
    if config.channels.cli.enabled && cli.message.is_none() {
        let boot_info = enclagent::boot_screen::BootInfo {
        ...
        enclagent::boot_screen::print_boot_screen(&boot_info);
    }
...
/// Called after `load_enclagent_env()`, so DATABASE_URL from `~/.enclagent/.env`
/// is already in the environment.
#[cfg(any(feature = "postgres", feature = "libsql"))]
fn check_onboard_needed() -> Option<&'static str> {
    let has_db = std::env::var("DATABASE_URL").is_ok()
        || std::env::var("LIBSQL_PATH").is_ok()
        || enclagent::config::default_libsql_path().exists();

    if !has_db {
        return Some("Database not configured");
    }

    // First run detection and provider-specific auth requirements.
    // For non-NEAR backends, onboarding should not require a NEAR AI session.
    let settings = enclagent::settings::Settings::load();
    let selected_backend = std::env::var("LLM_BACKEND")
        .ok()
        .or_else(|| settings.llm_backend.clone())
        .and_then(|raw| enclagent::settings::normalize_llm_backend(&raw))
        .unwrap_or_else(|| "nearai".to_string());

    if selected_backend == "nearai" && std::env::var("NEARAI_API_KEY").is_err() {
        let session_path = enclagent::llm::session::default_session_path();
...
async fn inject_channel_credentials(
    channel: &Arc<enclagent::channels::wasm::WasmChannel>,
