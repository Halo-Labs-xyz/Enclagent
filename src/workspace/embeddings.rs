...
pub trait EmbeddingProvider: Send + Sync {
    ...
    /// Probe provider health via the embeddings endpoint.
    ///
    /// Runs a minimal embedding request and validates returned vector size.
    async fn health_probe(&self) -> Result<(), EmbeddingError> {
        let embedding = self.embed("enclagent-health-probe").await?;
        if embedding.len() != self.dimension() {
            return Err(EmbeddingError::InvalidResponse(format!(
                "Probe dimension mismatch: expected {}, got {}",
                self.dimension(),
                embedding.len()
            )));
        }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_mock_embeddings_health_probe() {
        let provider = MockEmbeddings::new(32);
        provider.health_probe().await.unwrap();
    }
