...
use crate::agent::IntentAuditRecord;
...
impl WorkspaceStorage {
    fn database(&self) -> Option<Arc<dyn Database>> {
        match self {
            #[cfg(feature = "postgres")]
            Self::Repo(_) => None,
            Self::Db(db) => Some(Arc::clone(db)),
        }
    }
...
impl Workspace {
    ...
    /// Persist an intent audit record for replay.
    pub async fn persist_audit_record(
        &self,
        record: &IntentAuditRecord,
    ) -> Result<(), WorkspaceError> {
        let payload = record.workspace_document_payload();
        let serialized =
            serde_json::to_string_pretty(&payload).map_err(|err| WorkspaceError::SearchFailed {
                reason: format!("audit serialization failed: {}", err),
            })?;
        self.write(record.workspace_document_path(), &serialized)
            .await?;
        ...
        if let Some(db) = self.storage.database() {
            ...
            .await
            .map_err(|err| WorkspaceError::SearchFailed {
                reason: format!("persisting audit settings failed: {}", err),
            })?;
        }
