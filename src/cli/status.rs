...
pub async fn run_status_command() -> anyhow::Result<()> {
    ...
    if session_path.exists() {
        ...
        println!("not found (run `enclagent onboard`)");
    }
    ...
    match crate::tools::mcp::config::load_mcp_servers().await {
        Ok(servers) => {
            ...
            if enabled == 0 {
                ...
                let health = crate::tools::mcp::health::probe_enabled_servers(
                    servers.enabled_servers().cloned(),
                    std::time::Duration::from_secs(3),
                )
                .await;
                let healthy = health.iter().filter(|entry| entry.is_healthy()).count();
                println!(
                    "{} healthy / {} enabled / {} configured",
    ...
    println!(
        "\n  Config:      {}",
        crate::bootstrap::enclagent_env_path().display()
    );
