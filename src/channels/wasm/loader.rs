...
//!
//! Loads WASM channel modules from the filesystem (default: ~/.enclagent/channels/).
...
use crate::channels::wasm::wrapper::{CHANNEL_HOST_ABI_VERSION, CHANNEL_WIT_WORLD};
        ...
        let (capabilities, config_json, description, cap_file) =
            if let Some(cap_path) = capabilities_path {
                if cap_path.exists() {
                    ...
                    validate_host_compatibility(name, &cap_file)?;
...
fn validate_host_compatibility(
    ...
    let Some(compatibility) = cap_file.host_compatibility.as_ref() else {
        return Err(WasmChannelError::InvalidCapabilities(format!(
            "channel '{}' is missing host_compatibility metadata; rebuild channel with host metadata (expected wit_world='{}', host_abi_version={})",
            channel_name, CHANNEL_WIT_WORLD, CHANNEL_HOST_ABI_VERSION
        )));
    };

    if compatibility.wit_world != CHANNEL_WIT_WORLD {
        return Err(WasmChannelError::InvalidCapabilities(format!(
            "channel '{}' targets wit_world='{}' but host requires '{}'; rebuild channel against current WIT",
            channel_name, compatibility.wit_world, CHANNEL_WIT_WORLD
        )));
    }

    if compatibility.host_abi_version != CHANNEL_HOST_ABI_VERSION {
        return Err(WasmChannelError::InvalidCapabilities(format!(
            "channel '{}' targets host_abi_version={} but host requires {}; rebuild channel",
            channel_name, compatibility.host_abi_version, CHANNEL_HOST_ABI_VERSION
        )));
    }
