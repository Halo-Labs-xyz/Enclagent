...
/// Host-side WIT world name expected by channel components.
pub const CHANNEL_WIT_WORLD: &str = "sandboxed-channel";
/// Host-side compatibility version for channel capability metadata.
pub const CHANNEL_HOST_ABI_VERSION: u32 = 1;
...
impl ChannelStoreData {
    ...
    /// Render URL for logs without path/query/userinfo, to avoid leaking secrets.
...
impl near::agent::channel_host::Host for ChannelStoreData {
    ...
    fn http_request(
        ...
        tracing::info!(
            method = %method,
            target = %ChannelStoreData::redact_url_for_logs(&url),
