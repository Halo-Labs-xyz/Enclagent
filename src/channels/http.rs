...
use crate::agent::A2aCopyTradingMessage;
...
struct WebhookRequest {
    ...
    /// Optional WS-10 A2A copytrading contract payload.
    #[serde(default)]
    a2a_message: Option<A2aCopyTradingMessage>,
}
...
async fn webhook_handler(
    ...
    if let Some(ref contract) = req.a2a_message
        && let Err(error) = contract.validate()
    {
        return (
            ...
            Json(WebhookResponse {
                message_id: Uuid::nil(),
                status: "error".to_string(),
                response: Some(format!("Invalid a2a_message: {}", error)),
            }),
        );
    }
