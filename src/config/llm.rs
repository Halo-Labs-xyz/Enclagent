...
///
/// Defaults to `NearAi` to keep Enclagent close to the NEAR ecosystem.
...
pub struct NearAiConfig {
    ...
    /// Path to session file (default: ~/.enclagent/session.json)
...
impl LlmConfig {
    pub(crate) fn resolve(settings: &Settings) -> Result<Self, ConfigError> {
        ...
        let openai = if backend == LlmBackend::OpenAi {
            // API key may be injected later from encrypted secrets after DB init.
            let api_key = optional_env("OPENAI_API_KEY")?.map(SecretString::from);
            let model = optional_env("OPENAI_MODEL")?
                .or_else(|| settings.selected_model.clone())
                .unwrap_or_else(|| "gpt-4o".to_string());
            api_key.map(|api_key| OpenAiDirectConfig { api_key, model })
        } else {
            None
        };
        ...
        let anthropic = if backend == LlmBackend::Anthropic {
            ...
            api_key.map(|api_key| AnthropicDirectConfig {
                api_key,
                model,
                base_url,
            })
...
mod tests {
    ...
    fn ollama_uses_selected_model_when_ollama_model_env_unset() {
        ...
        assert_eq!(ollama.model, "glm-4.7-flash:latest");
    }
    ...
    #[test]
    fn openai_uses_selected_model_when_openai_model_unset() {
        let _guard = ENV_MUTEX.lock().expect("env mutex poisoned");
        clear_openai_env();
        // SAFETY: Under ENV_MUTEX.
        unsafe {
    ...
    #[test]
    fn openai_model_env_overrides_selected_model() {
        let _guard = ENV_MUTEX.lock().expect("env mutex poisoned");
        clear_openai_env();
        // SAFETY: Under ENV_MUTEX.
        unsafe {
    ...
    #[test]
    fn anthropic_uses_selected_model_when_anthropic_model_unset() {
        let _guard = ENV_MUTEX.lock().expect("env mutex poisoned");
        clear_anthropic_env();
        // SAFETY: Under ENV_MUTEX.
        unsafe {
    ...
    #[test]
    fn anthropic_model_env_overrides_selected_model() {
        let _guard = ENV_MUTEX.lock().expect("env mutex poisoned");
        clear_anthropic_env();
        // SAFETY: Under ENV_MUTEX.
        unsafe {
            std::env::set_var("ANTHROPIC_API_KEY", "test-anthropic-key");
            std::env::set_var("ANTHROPIC_MODEL", "claude-sonnet-4-20250514");
        }
    ...
    #[test]
    fn anthropic_base_url_env_is_applied() {
        let _guard = ENV_MUTEX.lock().expect("env mutex poisoned");
        clear_anthropic_env();
        // SAFETY: Under ENV_MUTEX.
        unsafe {
        ...
        let settings = Settings {
            llm_backend: Some("anthropic".to_string()),
            selected_model: Some("MiniMax-M2.5".to_string()),
            ..Default::default()
        };
        ...
        assert_eq!(anthropic.model, "MiniMax-M2.5");
