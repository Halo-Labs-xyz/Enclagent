...
pub struct DatabaseConfig {
    ...
    // -- libSQL fields --
    /// Path to local libSQL database file (default: ~/.enclagent/enclagent.db).
...
impl DatabaseConfig {
    pub(crate) fn resolve() -> Result<Self, ConfigError> {
        ...
        // DATABASE_URL is loaded from ~/.enclagent/.env via dotenvy early in startup.
        let url = optional_env("DATABASE_URL")?
            ...
            .ok_or_else(|| ConfigError::MissingRequired {
                ...
                hint: "Run 'enclagent onboard' or set DATABASE_URL environment variable"
                    .to_string(),
            })?;
...
/// Default libSQL database path (~/.enclagent/enclagent.db).
