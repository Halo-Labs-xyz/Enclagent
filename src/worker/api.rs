...
//!
//! Every request includes a bearer token from `ENCLAGENT_WORKER_TOKEN` env var.
...
impl WorkerHttpClient {
    ...
    ///
    /// Reads `ENCLAGENT_WORKER_TOKEN` from the environment.
    pub fn from_env(orchestrator_url: String, job_id: Uuid) -> Result<Self, WorkerError> {
        let token =
            std::env::var("ENCLAGENT_WORKER_TOKEN").map_err(|_| WorkerError::MissingToken)?;
