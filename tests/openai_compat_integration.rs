...
impl LlmProvider for MockLlmProvider {
    ...
    async fn complete(&self, req: CompletionRequest) -> Result<CompletionResponse, LlmError> {
        ...
        let user_msg = req
            ...
            .iter()
            .rev()
            .find(|m| m.role == enclagent::llm::Role::User)
    ...
    async fn complete_with_tools(
        ...
        if let Some(tool) = req.tools.first() {
            Ok(ToolCompletionResponse {
                content: None,
                tool_calls: vec![enclagent::llm::ToolCall {
...
async fn start_test_server() -> Option<(SocketAddr, Arc<GatewayState>)> {
    ...
    match start_server(addr, state.clone(), AUTH_TOKEN.to_string()).await {
        Ok(bound_addr) => Some((bound_addr, state)),
...
async fn test_chat_completions_basic() {
    let Some((addr, _state)) = start_test_server().await else {
        return;
    };
...
async fn test_chat_completions_streaming() {
    ...
    assert_eq!(
        resp.headers()
            .get("x-enclagent-streaming")
            .and_then(|v| v.to_str().ok()),
        Some("simulated"),
        "Expected x-enclagent-streaming: simulated header"
    );
...
async fn test_no_llm_provider_returns_503() {
    ...
    let addr: SocketAddr = "127.0.0.1:0".parse().unwrap();
    let bound_addr = match start_server(addr, state, AUTH_TOKEN.to_string()).await {
        Ok(bound_addr) => bound_addr,
        Err(e) if is_bind_permission_error(&e) => return,
        Err(e) => panic!("Failed to start test server: {e:?}"),
    };
