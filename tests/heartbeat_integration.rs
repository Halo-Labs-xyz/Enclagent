...
async fn test_heartbeat_end_to_end() {
    ...
    let _ = tracing_subscriber::fmt()
        .with_env_filter("enclagent=debug")
        .try_init();
    ...
    let hb_config = enclagent::agent::HeartbeatConfig::default();
    ...
    println!("=== Result ===\n");
    match &result {
        enclagent::agent::HeartbeatResult::Ok => {
            println!("HeartbeatResult::Ok");
            println!("  LLM responded HEARTBEAT_OK, nothing needs attention.");
        }
        enclagent::agent::HeartbeatResult::NeedsAttention(msg) => {
            println!("HeartbeatResult::NeedsAttention");
            println!("  Message:\n{}", msg);
        }
        enclagent::agent::HeartbeatResult::Skipped => {
        ...
        enclagent::agent::HeartbeatResult::Failed(err) => {
