...
use enclagent::agent::{Agent, AgentDeps};
use enclagent::channels::{ChannelManager, IncomingMessage};
use enclagent::config::AgentConfig;
use enclagent::llm::LlmProvider;
use enclagent::safety::SafetyLayer;
use enclagent::tools::ToolRegistry;
...
struct TaskRunParams<'a> {
    ...
    additional_tools: &'a [Arc<dyn enclagent::tools::Tool>],
}
...
impl BenchRunner {
    ...
    pub async fn run(
        ...
        if self.config.parallelism <= 1 {
            ...
            let shared_tools: Arc<[Arc<dyn enclagent::tools::Tool>]> =
...
async fn run_task_isolated(params: TaskRunParams<'_>) -> TaskResult {
    ...
    let cost_guard = Arc::new(enclagent::agent::cost_guard::CostGuard::new(
        enclagent::agent::cost_guard::CostGuardConfig::default(),
    ));
    ...
    let deps = AgentDeps {
        ...
        skill_registry: None,
        skills_config: enclagent::config::SkillsConfig::default(),
        hooks: Arc::new(enclagent::hooks::HookRegistry::new()),
        cost_guard,
    };
