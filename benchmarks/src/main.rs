...
#[derive(Parser)]
#[command(name = "enclagent-bench", about = "Enclagent benchmarking harness")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::registry()
        .with(
            EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| EnvFilter::new("enclagent_bench=info,enclagent=warn")),
        )
    ...
    match cli.command {
        ...
        Commands::Run {
            ...
            // Initialize enclagent LLM provider
            let enclagent_config = enclagent::Config::from_env().await.map_err(|e| {
                anyhow::anyhow!(
                    "Failed to load enclagent config: {}. Make sure .env is configured.",
                    e
                )
            })?;

            let session = enclagent::llm::create_session_manager(enclagent::llm::SessionConfig {
                auth_base_url: enclagent_config.llm.nearai.auth_base_url.clone(),
                session_path: enclagent_config.llm.nearai.session_path.clone(),
            })
            .await;
            session.ensure_authenticated().await?;

            let llm = enclagent::llm::create_llm_provider(&enclagent_config.llm, session)?;
            let safety = Arc::new(enclagent::safety::SafetyLayer::new(
                &enclagent_config.safety,
            ));
