...
This guide covers configuring the Telegram channel for Enclagent, including DM pairing for access control.

## Overview

The Telegram channel lets you interact with Enclagent via Telegram DMs and groups. It supports:
...
- Enclagent installed and configured (`enclagent onboard`)
...
2. Bot replies: `To pair with this bot, run: enclagent pairing approve telegram ABC12345`
3. You run: `enclagent pairing approve telegram ABC12345`
...
enclagent pairing list telegram

# List as JSON
enclagent pairing list telegram --json

# Approve a user by code
enclagent pairing approve telegram ABC12345
```

### Configuration

Edit `~/.enclagent/channels/telegram.capabilities.json` (or the config injected by the host):
...
- **Secrets store**: `enclagent` CLI (if available)
...
- Set `bot_username` in config to your bot's username (e.g., `MyEnclagentBot`)
- Ensure the message contains `@YourBot` or starts with `/`

### "Connection refused" when starting

- For webhook mode: Start your tunnel before `enclagent run`
