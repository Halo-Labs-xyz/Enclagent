...
- Uses a slim verifiable runtime build profile (`Dockerfile` + `Dockerfile.ecloud`: `--profile ecloud --no-default-features --features libsql`; `profile.ecloud` is compile-speed optimized with `opt-level=0` and high codegen units) so EigenCloud source builds match enclave runtime database mode and stay within upstream build-duration limits. This profile intentionally excludes `system-keyring` so headless enclave builds do not compile Linux desktop keyring crates (`secret-service`, `zbus`) and must source `SECRETS_MASTER_KEY` from environment.
