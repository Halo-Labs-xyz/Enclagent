...
- Uses a slim verifiable runtime build profile (`Dockerfile` + `Dockerfile.ecloud`: `--no-default-features --features libsql`) so EigenCloud source builds match the enclave runtime database mode and remain under upstream build-duration limits.
...
- If the queued `build_id` belongs to a different repo/commit than the requested source provenance, provisioner treats it as queue contention and continues retries after it exits, instead of failing the user session on unrelated build failure.
