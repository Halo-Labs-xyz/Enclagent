...
This guide covers how to build WASM channel modules for Enclagent.
...
version = "0.1.0"
edition = "2021"
description = "My messaging platform channel for Enclagent"
...
`host_compatibility` is required. Enclagent validates this handshake at load
time and will refuse to start incompatible channels with a rebuild error.

## Building and Deploying

### Supply Chain Security: No Committed Binaries

**Do not commit compiled WASM binaries.** They are a supply chain risk — the binary in a PR may not match the source. Enclagent builds channels from source:
...
# Install (or use enclagent onboard to install bundled channel)
mkdir -p ~/.enclagent/channels
cp channels-src/telegram/telegram.wasm channels-src/telegram/telegram.capabilities.json ~/.enclagent/channels/
```

**Note**: The main Enclagent binary bundles `telegram.wasm` via `include_bytes!`. When modifying the Telegram channel source, run `./channels-src/telegram/build.sh` **before** building the main crate, so the updated WASM is included.
...
# Deploy to ~/.enclagent/channels/
cp target/wasm32-wasip2/release/my_channel.wasm ~/.enclagent/channels/my-channel.wasm
cp my-channel.capabilities.json ~/.enclagent/channels/
```
