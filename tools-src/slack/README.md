...
A standalone WASM component that provides Slack integration for Enclagent. This serves as both a functional tool and a template for building custom WASM tools.
...
```bash
mkdir -p ~/.enclagent/tools
cp target/wasm32-wasip2/release/slack_tool.wasm ~/.enclagent/tools/slack.wasm
cp slack.capabilities.json ~/.enclagent/tools/
```
...
```bash
enclagent secret set slack_bot_token "xoxb-your-token-here"
```
...
```bash
enclagent secret set slack_bot_token "xoxb-..."
```
