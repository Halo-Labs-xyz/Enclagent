...
3. Store the token:
   ...
   ```
   enclagent secret set github_token YOUR_TOKEN
   ```
...
```text
GitHub token not found in secret store. Set it with: enclagent secret set github_token <token>...
```
...
- Update the token: `enclagent secret set github_token NEW_TOKEN`.
